...
class MarkdownAdapter(Adapter):
    ...
    def list(self, tier=None, query=None):
        ...
        return _MdQuery(q, self.me).run(self._all_tickets())
...
class _MdQuery:
    """Tiny JQL subset evaluator shared in spirit with the Jira adapter so the
    same config [queries] strings work on a markdown board. Supports:
        field = "value"          field != "value"
        assignee = currentUser()  assignee is EMPTY
        clauses joined by AND
        optional trailing: ORDER BY field [ASC|DESC][, field [ASC|DESC]]
    Field names map to ticket attributes; `status`/`priority`/`assignee`/`type`.
    """

    _FIELDS = {"status", "priority", "assignee", "type", "key"}
    ...
    def _split_and(s: str) -> list[str]:
        # case-insensitive split on AND surrounded by spaces
        out, buf, i = [], [], 0
        tokens = s.split(" ")
        for tok in tokens:
            if tok.upper() == "AND":
    ...
    def _match(self, t: Ticket, clause: str) -> bool:
        if " is EMPTY" in clause or " IS EMPTY" in clause.upper():
