...
4. Read existing code for each affected package (route handlers, types, schemas, tests, API specs). Keep the package→file mapping in the project's own conventions doc.
5. Classify the change: new endpoint, bug fix, new feature, refactor, or shared-lib change.
...
6. Produce a markdown plan with these sections:
...
7. Validate each acceptance criterion is covered by the plan. Note any gap.
8. If estimated size > 400 lines, comment on the ticket recommending sub-tasks, then pause for confirmation before proceeding.
